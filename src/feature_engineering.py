...
FEATURE_CONFIG_FILE = (
    PROJECT_ROOT / "data" / "feature_config.json"
)
...
FEATURES_SNAPSHOTS_DIR = (
    SNAPSHOTS_DIR / "features"
)

CONFIG_SNAPSHOTS_DIR = (
    SNAPSHOTS_DIR / "configs"
)
...
def load_filtered_dataset(file_path: Path) -> pd.DataFrame:
    ...
    if not file_path.exists():
        raise FileNotFoundError(
            f"Filtered dataset not found: {file_path}"
        )

    df = pd.read_csv(file_path)

    logger.info(
        "Dataset loaded (%s rows, %s columns)",
        len(df),
        len(df.columns)
    )
...
def create_imputation_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["duration_imputed_flag"] = (
        (df["duration_years"] <= 0)
        | (df["duration_years"] > 10)
        | (df["duration_years"].isna())
    )

    df["monthly_income_imputed_flag"] = (
        (df["monthly_income"] <= 0)
        | (df["monthly_income"].isna())
    )

    df["annual_cost_imputed_flag"] = (
        (df["annual_cost"] <= 0)
        | (df["annual_cost"].isna())
    )
    ...
    df["admission_rate_imputed_flag"] = (
        (df["admission_rate"] <= 0)
        | (df["admission_rate"] > 90)
        | (df["admission_rate"].isna())
    )
...
def duration_fallback(row):
    ...
    if row["institution_type"] == "Instituto":
        return FEATURE_CONFIG[
            "duration_institute_fallback"
        ]

    return FEATURE_CONFIG[
        "duration_university_fallback"
    ]
...
def impute_variable(
    ...
    valid_data = df.loc[
        pd.notna(df[target_column])
    ]

    median_cf_inst = (
        valid_data
        .groupby(
            ["career_family", "institution_type"]
        )[target_column]
        .median()
    )
...
def create_norm_features(
    ...
    df["income_norm"] = minmax_scale(
        np.log1p(
            df["monthly_income_imputed"]
        )
    )

    df["admission_norm"] = minmax_scale(
        df["admission_rate_imputed"]
    )

    df["cost_norm"] = (
        1 - minmax_scale(
            np.log1p(
                df["annual_cost_imputed"]
            )
        )
    )
...
def save_feature_config() -> None:
    ...
    FEATURE_CONFIG_FILE.parent.mkdir(
        parents=True,
        exist_ok=True
    )
...
def save_config_snapshot() -> None:
    ...
    snapshot_file = (
        CONFIG_SNAPSHOTS_DIR
        / f"feature_config_{timestamp}.json"
    )
    ...
    logger.info(
        "Feature config snapshot saved to %s",
        snapshot_file
    )
